...
import { ArrowLeft, Crown, Eye, EyeOff, Loader2 } from "lucide-react"
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      // 2. GANTI ALERT ERROR JADI TOAST MERAH
      toast.error("Gagal Daftar", {
        description: error.message
      })
      setIsLoading(false)
    } else {
      // 3. GANTI ALERT SUKSES JADI TOAST HIJAU
      toast.success("Pendaftaran Berhasil!", {
        description: "Akunmu sudah aktif. Silakan login.",
        duration: 4000,
      })
      
      // Delay dikit biar user sempet baca notif sebelum pindah halaman
      setTimeout(() => {
         router.push("/login")
      }, 1500)
    }
      ...
      <div className="relative z-30 w-full max-w-[450px] px-6 animate-in fade-in zoom-in duration-500">
        ...
        <div className="backdrop-blur-xl bg-black/30 border border-white/10 shadow-2xl rounded-3xl p-8 text-white">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  required
                  placeholder="Andi" 
                  className="bg-white/10 border-white/10 text-white focus-visible:ring-gold h-10 rounded-xl" 
                />
              </div>
              <div className="space-y-2">
            ...
            <Button type="submit" disabled={isLoading} className="w-full bg-gold hover:bg-yellow-500 text-navy font-bold h-11 rounded-xl shadow-lg mt-2">
